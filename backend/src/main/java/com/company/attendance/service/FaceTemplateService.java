 ...
 *
 * Handles face template creation, retrieval, and enrollment operations by coordinating
 * with the edge service for face processing.
 */
@Service
...
public class FaceTemplateService {
	...
	public int enrollFromImages(Employee employee, List<MultipartFile> images, String description) {
		...
		for (int i = 0; i < images.size(); i++) {
			...
			try {
				log.debug("Processing image {} of {} for employee {}", i + 1, images.size(),
		...
		log.info("Enrollment completed for employee {}: {} templates created, {} errors", employee.getEmployeeCode(),
				templatesCreated, errors.size());
	...
	private List<FaceTemplate> processImageWithEdgeService(Employee employee, MultipartFile image, String description)
		...
		for (JsonNode embedding : embeddings) {
			...
			if (embedding.get("is_live").asBoolean() && embedding.get("liveness_score").asDouble() >= 0.5
				...
				log.debug("Skipped low quality face: confidence={}, liveness={}, live={}",
						embedding.get("confidence").asDouble(), embedding.get("liveness_score").asDouble(),
