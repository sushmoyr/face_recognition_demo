...
public class DeviceController {
	...
	public ResponseEntity<Page<DeviceResponse>> getAllDevices(@RequestParam(required = false) String location,
	...
	public ResponseEntity<?> heartbeat(@Valid @RequestBody HeartbeatRequest request) {
		try {
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Heartbeat failed: " + e.getMessage());
		}
	}
	...
	public ResponseEntity<?> getDeviceStats() {
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error retrieving device statistics");
		}
	}
	...
	@PreAuthorize("hasRole('ADMIN')")
	public ResponseEntity<DeviceResponse> updateDeviceStatus(@PathVariable UUID id, @RequestParam boolean active) {

		Device device = deviceService.updateStatus(id, active);

		log.info("Updated device status: {} ({}) -> {}", device.getName(), device.getDeviceCode(),
