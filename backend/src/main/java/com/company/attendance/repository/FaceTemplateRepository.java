 ...
 * 
 * Provides operations for face template management including
...
public interface FaceTemplateRepository extends JpaRepository<FaceTemplate, UUID> {
     ...
     */
    @Query("SELECT ft FROM FaceTemplate ft " +
           "WHERE ft.employee.id = :employeeId " +
           "AND ft.isActive = true " +
           "AND ft.qualityScore >= :qualityThreshold " +
           "ORDER BY ft.qualityScore DESC")
     ...
     */
    @Query(value = "SELECT * FROM face_templates ft " +
                   "WHERE ft.employee_id = :employeeId " +
                   "AND ft.is_active = true " +
                   "ORDER BY ft.quality_score DESC " +
                   "LIMIT :limit", 
           nativeQuery = true)
    List<FaceTemplate> findTopByQuality(@Param("employeeId") UUID employeeId, 
                                       @Param("limit") int limit);
    
    /**
     * Find similar face templates using cosine similarity.
    ...
    @Query(value = "SELECT ft.*, (1 - (ft.embedding <=> CAST(:embedding AS vector))) as similarity " +
                   ...
                   "AND (1 - (ft.embedding <=> CAST(:embedding AS vector))) >= :threshold " +
                   "ORDER BY similarity DESC " +
                   "LIMIT :limit", 
           nativeQuery = true)
    List<Object[]> findSimilarTemplates(@Param("embedding") String embedding, 
    ...
    @Query(value = "SELECT ft.*, (1 - (ft.embedding <=> CAST(:embedding AS vector))) as similarity " +
                   ...
                   "LIMIT 1", 
           nativeQuery = true)
    Object[] findMostSimilar(@Param("embedding") String embedding, 
                            @Param("threshold") Double threshold);
     ...
     */
    @Query("SELECT COUNT(ft) FROM FaceTemplate ft " +
           "WHERE ft.employee.id = :employeeId AND ft.isActive = true")
     ...
     */
    @Query(value = "UPDATE face_templates SET is_active = false " +
                   ...
                   ")", 
           nativeQuery = true)
    void deactivateOldTemplates(@Param("employeeId") UUID employeeId, 
