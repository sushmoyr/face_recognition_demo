...
public class DeviceService {
    ...
    public Device create(DeviceCreateRequest request) {
        ...
        // Note: Other fields like ipAddress, macAddress etc. don't exist in the Device entity
        device.setStatus(request.getActive() != null && request.getActive() ? 
            Device.DeviceStatus.ACTIVE : Device.DeviceStatus.ACTIVE);
    ...
    public Device update(UUID id, DeviceUpdateRequest request) {
        ...
        // Note: Other fields like ipAddress, macAddress, version etc. don't exist in current Device entity
        
        if (request.getActive() != null) {
            device.setStatus(request.getActive() ? 
    ...
    public Device updateStatus(UUID id, boolean active) {
        ...
        Device savedDevice = deviceRepository.save(device);
        log.info("Updated device status: {} at {} -> {}", 
            savedDevice.getName(), savedDevice.getLocation(), 
