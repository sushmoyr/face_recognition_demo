...
public class RecognitionEventService {
    ...
    public RecognitionEvent processRecognition(RecognitionEventRequest request) {
        log.debug("Processing recognition event for device: {} with top candidate: {}", 
                 request.getDeviceId(), request.getTopCandidateEmployeeId());
        
        // Find employee by ID (if provided)
        Employee employee = null;
        if (request.getTopCandidateEmployeeId() != null) {
            employee = employeeRepository.findById(request.getTopCandidateEmployeeId())
                .orElse(null);
        }
        ...
        if (recognitionEventRepository.existsByDedupHash(dedupHash)) {
            log.info("Duplicate recognition event detected - hash: {} for employee: {} device: {}", 
                    dedupHash, employeeCode, deviceId);
        ...
        if (employee != null && event.isValidMatch()) {
            try {
                ...
                log.info("Processed attendance for recognition event: {} employee: {}", 
                        event.getId(), employee.getEmployeeCode());
            } catch (Exception e) {
     ...
     */
    private RecognitionEvent createRecognitionEventFromRequest(RecognitionEventRequest request, 
                                                               Employee employee, 
                                                               Device device, 
                                                               String dedupHash) {
