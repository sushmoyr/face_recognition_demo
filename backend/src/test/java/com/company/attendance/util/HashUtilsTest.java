...
class HashUtilsTest {
	...
	void testIsWithinDeduplicationWindow() {
		...
		assertTrue(hashUtils.isWithinDeduplicationWindow(time2, time1), "Should work regardless of time order");
	}
	...
	void testGenerateImageHash_WithRealFile() throws IOException {
		...
		// When
		String hash1 = hashUtils.generateRecognitionHash(tempFile.getAbsolutePath(), "EMP001", "device-123",
				Instant.now());
		String hash2 = hashUtils.generateRecognitionHash(tempFile.getAbsolutePath(), "EMP001", "device-123",
				Instant.now());

		// Then
		assertNotNull(hash1);
		assertNotNull(hash2);
		// Note: These may not be equal due to time bucketing, but both should be valid
		// hashes
	...
	void testHashStability_AcrossMultipleCalls() {
		...
		for (int i = 1; i < 10; i++) {
			assertEquals(hashes[0], hashes[i], "Hash should be stable across multiple calls with same input");
		}
	}
