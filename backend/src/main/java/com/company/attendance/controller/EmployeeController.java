 ...
 * 
 * Provides CRUD operations for employees and face enrollment functionality.
...
public class EmployeeController {
    ...
    public ResponseEntity<?> enrollFace(
        ...
        try {
            ...
            for (MultipartFile image : images) {
                if (!isValidImageFile(image)) {
                    return ResponseEntity.badRequest()
                        .body("Invalid image file: " + image.getOriginalFilename());
                }
            }
            ...
            log.info("Enrolled {} face templates for employee: {} ({})", 
                templatesCreated, employee.getFullName(), employee.getEmployeeCode());
            
            return ResponseEntity.ok()
                .body("Successfully enrolled " + templatesCreated + " face templates");
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'HR')")
    public ResponseEntity<?> enrollFaceEmbedding(
            @PathVariable UUID id,
        ...
        try {
            ...
            log.info("Enrolled face embedding for employee: {} ({})", 
    ...
    public ResponseEntity<?> getFaceTemplates(@PathVariable UUID id) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body("Error retrieving face templates");
        }
    }
    ...
    public ResponseEntity<?> getAllTemplates() {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body("Error retrieving templates");
        }
    }
    ...
    public ResponseEntity<EmployeeResponse> updateEmployeeStatus(
        ...
        log.info("Updated employee status: {} ({}) -> {}", 
            employee.getFullName(), employee.getEmployeeCode(), active ? "ACTIVE" : "INACTIVE");
