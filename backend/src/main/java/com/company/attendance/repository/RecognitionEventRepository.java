 ...
 * 
 * Manages face recognition events including deduplication
 * queries and processing status tracking.
 */
@Repository
public interface RecognitionEventRepository extends JpaRepository<RecognitionEvent, UUID> {
     ...
     */
    @Query("SELECT re FROM RecognitionEvent re WHERE " +
           "re.employee.id = :employeeId " +
           "AND re.device.id = :deviceId " +
           "AND re.capturedAt > :timeWindow " +
           "AND re.status != 'DUPLICATE'")
    ...
    @Query("SELECT re FROM RecognitionEvent re WHERE " +
           "re.status = 'PROCESSED' " +
           "AND re.attendanceRecord IS NULL " +
           "AND re.employee IS NOT NULL " +
           "ORDER BY re.capturedAt")
    ...
    @Query("SELECT re.device.deviceCode, COUNT(re) FROM RecognitionEvent re " +
           "WHERE re.capturedAt BETWEEN :startTime AND :endTime " +
           "GROUP BY re.device.deviceCode")
    List<Object[]> countByDeviceInTimeRange(@Param("startTime") Instant startTime,
    ...
    @Query("SELECT re.employee.employeeCode, COUNT(re) FROM RecognitionEvent re " +
           ...
           "GROUP BY re.employee.employeeCode")
