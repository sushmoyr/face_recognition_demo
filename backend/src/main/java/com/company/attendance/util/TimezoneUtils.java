...
public class TimezoneUtils {
	...
	public Instant businessDateTimeToUtc(LocalDateTime businessDateTime) {
		return businessDateTime.atZone(BUSINESS_TIMEZONE).withZoneSameInstant(UTC_TIMEZONE).toInstant();
	}
	...
	/**
	 * Calculate duration between two times on the same date. Handles overnight shifts by
	 * adjusting end time if it's before start time.
	 */
	...
	/**
	 * Adjust time for overnight calculation. If the time is before noon and it's an
	 * overnight shift, assume it's next day.
	 */
	public LocalTime adjustTimeForOvernightShift(LocalTime time, LocalTime shiftStart) {
		// If time is significantly before shift start (e.g., shift starts at 22:00, time
		// is 02:00),
	...
	/**
	 * Check if daylight saving time is in effect (Bangladesh doesn't use DST, but keeping
	 * for completeness).
	 */
