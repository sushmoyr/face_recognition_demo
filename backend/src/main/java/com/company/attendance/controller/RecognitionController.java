 ...
 * 
 * Handles face recognition events from edge devices and provides
...
public class RecognitionController {
    ...
    public ResponseEntity<?> ingestRecognition(@Valid @RequestBody RecognitionEventRequest request) {
        try {
            ...
            log.info("Recognition event processed: {} for employee {}", 
                event.getId(), event.getEmployee() != null ? event.getEmployee().getEmployeeCode() : "UNKNOWN");
            
            return ResponseEntity.status(HttpStatus.CREATED)
                .body(convertToResponse(event));
     ...
     */
    @GetMapping
    @Operation(summary = "List recognition events", description = "Get paginated list of recognition events with filtering")
    ...
    public ResponseEntity<Page<RecognitionEventResponse>> getRecognitionEvents(
        ...
        Page<RecognitionEvent> events = recognitionEventService.findAll(
            employeeId, deviceId, startDate, endDate, minConfidence, pageable);
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'HR', 'VIEWER')")
    public ResponseEntity<?> getRecognitionStats(
            @RequestParam(required = false) LocalDate startDate,
            @RequestParam(required = false) LocalDate endDate) {
    ...
    public ResponseEntity<Page<RecognitionEventResponse>> getUnmatchedRecognitions(
        ...
        Page<RecognitionEvent> events = recognitionEventService.findUnmatched(
